...
const CreateBook = ({ router }) => {
    ...
    const [body, setBody] = useState(bookFromLS());
    ...
    const publishBook = e => {
        e.preventDefault();
        // console.log('ready to publishBook');
        createBook(formData, token).then(data => {
            if (data.error) {
                setValues({ ...values, error: data.error });
            } else {
                setValues({ ...values, title: '', error: '', success: `A new book titled "${data.title}" is created` });
                setBody('');
                setCategories([]);
                setTags([]);
            }
    ...
    const handleBody = e => {
        ...
        */
        useState(localStorage.setItem('book', JSON.stringify(e)));
    };
